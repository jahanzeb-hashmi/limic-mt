        ...
        void * arg)
{
    ...
    switch (op_code) {
        ...
        case LIMIC_RX:
            ...
            frag_req.needed_workers = req.needed_workers;
            frag_req.pool_threshold = req.pool_threshold;
            ...
            while (len_left > 0) { 
                ...
                maplist = limic_get_pages(&frag_lu, 0);
...
/*
 * module initialization - called only once when module loaded
 * do the context specific initialization here
 */
...
int limic_init(void)
{
    ...
    /* proceed to do remaining things */
...
/*
 * called only once. Do the context clean-up here
 */
...
void limic_exit(void)
{
     ...
     * 2. cleanup pools
     * 3. cleanup lists
     * 4. free all the necessary allocations 
     */
   
    struct Wrapper_context *ctx;
    printk ("inside limic_exit \n"); 
    
    ctx = GET_CONTEXT();
    /* cleanup all pools and related allocs in conte */
    cleanup_pools_resources (ctx);
    
    /* after cleanup, destroy context */
    DESTROY_CONTEXT ();
